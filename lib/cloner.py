...
class ChannelCloner:
    ...
    async def clone_supergroup_with_channels(
        ...
        try:
            ...
            print(f"{CYAN}Iniciando clonagem completa do supergrupo: {source_dialog.name}")
    ...
    async def _clone_related_channels(
        ...
        for i, channel_dialog in enumerate(related_channels):
            try:
                ...
                print(f"{CYAN}Clonando canal: {channel_dialog.name} -> {channel_name}")
                ...
                if cloned_channel:
                    ...
                    results['errors'].append(f"Falha ao clonar canal: {channel_dialog.name}")
                ...
                try:
                    ...
                    if hasattr(entity, 'broadcast') and entity.broadcast:
                        if self._is_channel_related(dialog.name, base_words):
                            ...
                            print(f"{BLUE}Canal relacionado encontrado: {dialog.name}")
                        
                        elif await self._is_channel_linked_to_group(source_entity, entity):
                            related_channels.append(dialog)
                            print(f"{BLUE}Canal linkado encontrado: {dialog.name}")
    ...
    def _is_channel_related(self, channel_name: str, base_words: List[str]) -> bool:
        ...
        name_similarity = sum(1 for word in base_words if word in channel_name_lower)
        ...
        try:
            ...
            for channel in results['channels']:
                try:
                    ...
                    print(f"{YELLOW}Aviso: Não foi possível linkar canal {channel.title}: {e}")
    ...
    async def clone_channel(
        ...
        try:
            ...
            is_broadcast = hasattr(source_entity, 'broadcast') and source_entity.broadcast
            ...
            print(f"{GREEN}Novo canal criado: {new_channel.title} (ID: {new_channel.id})")
    ...
    async def _create_channel(
        ...
        result = await self.client(functions.channels.CreateChannelRequest(
            title=title,
            about=about or (full_info.full_chat.about if full_info.full_chat.about else ""),
    ...
    async def _copy_messages(self, source_entity, target_channel, limit: int) -> None:
        ...
        try:
            ...
            if is_protected:
                ...
                print(f"{YELLOW}Apenas texto será copiado, mídia será substituída por descrição")
            ...
            if skipped_media > 0:
                print(f"{YELLOW}Total de mídia protegida substituída: {skipped_media}")
        ...
        for message in messages:
            try:
                ...
                if copied_count % 10 == 0:
                    ...
                    if skipped_media > 0:
                        print(f"{YELLOW}Mídia protegida substituída: {skipped_media}")
                    time.sleep(1)
    ...
    async def _handle_message_error(
        ...
        try:
            ...
            print(f"{RED}Erro crítico ao processar mensagem (pulando): {fallback_error}")
